...
export var QueryFunctionEnum = (function (DataTypeEnum) {
  ...
  function EnumDefs() {
    ...
    this.COUNT = new QueryFunction(
      ...
      DataTypeEnum.STRING,
      DataTypeEnum.NUMBER,
    ); // TODO: Input can be any single value dataType
    this.UPPER = new QueryFunction(
      "UPPER",
      "Upper",
      "upper",
      false,
    ...
    this.LENGTH = new QueryFunction(
      ...
      DataTypeEnum.STRING,
      DataTypeEnum.NUMBER,
    ); // length(s) -- returns numeric value
    this.ABS = new QueryFunction(
      "ABS",
      "Abs",
      "abs",
      false,
    ...
    this.SIZE = new QueryFunction(
      ...
      DataTypeEnum.NUMBER,
    ); // size(collection) -- returns an integer (0 if empty)

    this.getAggregateFunctionValueList = () => {
      const valueArray = [];
    ...
    this.getAggregateFunctionValueListForInputType = (inputDataType) => {
      const valueArray = [];
      if (inputDataType) {
        Object.keys(this).map((key) => {
          const item = (this as any)[key];
          if (item === QueryFunctionEnum.COUNT) {
            if (
              inputDataType !== DataTypeEnum.OBJECT &&
              inputDataType !== DataTypeEnum.LOB &&
              inputDataType !== DataTypeEnum.COLLECTION
            ) {
              valueArray.push(item);
            }
          } else {
            if (
              ...
              item.isAggregate() &&
              item.getInputDataType() === inputDataType
            ) {
              valueArray.push(item);
            }
          }
    ...
    this.getQueryFunctionValueList = () => {
      const valueArray = [];
      Object.keys(this).map((key) => {
        const item = (this as any)[key];
        if (typeof item !== "function" && !item.isAggregate()) {
    ...
    this.getQueryFunctionValueListForInputType = (inputDataType) => {
      const valueArray = [];
      if (inputDataType) {
        Object.keys(this).map((key) => {
          const item = (this as any)[key];
