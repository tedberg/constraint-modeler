...
export default class ConstraintModel extends QueryElementModel {
  ...
  isValid() {
    if (this.verifiedValidity) {
      ...
      // Do a more basic check of syntax.
      if (
        typeof this.key === "undefined" ||
        this.key === null ||
        typeof this.comparisonType === "undefined" ||
        ...
        switch (this.comparisonType.expectedNumberOfObjectValues) {
          ...
          default:
            if (typeof this.valueArray === "undefined" || this.valueArray === null) {
              ...
              return (
                typeof this.valueArray[0] !== "undefined" &&
                this.valueArray[0] !== null &&
                typeof this.valueArray[1] !== "undefined" &&
                this.valueArray[1] !== null
              );
            }
        }
      }
  ...
  setValuesFromToken(constraintToken) {
    ...
    if (keyValuePair.length === 2) {
      ...
      if (
        keyValuePair[1] === ComparisonTypeEnum.NULL.alias ||
        keyValuePair[1] === ComparisonTypeEnum.NOT_NULL.alias ||
        keyValuePair[1] === ComparisonTypeEnum.EMPTY.alias ||
        keyValuePair[1] === ComparisonTypeEnum.NOT_EMPTY.alias
      ) {
  ...
  renderQueryString(applySpecialHandlerConversions) {
    ...
    if (typeof applySpecialHandlerConversions === "undefined") {
      log.error(
        "ConstraintModel.renderQueryString - argument: applySpecialHandlerConversions was undefined.",
      );
    }

    if (applySpecialHandlerConversions) {
      // Mutate constraint names for back end compatibility. False for preserving saved filter definition.
  ...
  isSpecialHandlerApplicable() {
    if (this.specialHandlerFunction && this.specialHandlerFunction.applicableComparisonTypeArray) {
      ...
      if (
        this.specialHandlerFunction.applicableComparisonTypeArray.indexOf(this.comparisonType) > -1
      ) {
        return true;
      }
    }
