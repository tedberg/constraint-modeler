...
test('Creates an Age Greater Than 25 constraint with projections', async ({ page }) => {
  ...
  await firstProjection.locator('[data-testid="projection-property-menu"] button').click();
  ...
  await secondProjection.locator('[data-testid="projection-property-menu"] button').click();
  await page.getByRole('menuitem', { name: /^Age$/ }).click();
